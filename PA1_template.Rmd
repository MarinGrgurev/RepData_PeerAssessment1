...
The mean total number of steps taken per day is `r format(mean(data.NA.omit[,list(total=sum(steps)), by="date"][[2]]))`.  
...
On average, across all the days in the dataset `r data.NA.omit[,list(total=mean(steps)), by="interval"][total==max(total)][[1]]`th interval contains maximum number of steps (`r data.NA.omit[,list(total=mean(steps)), by="interval"][total==max(total)][[2]]`).
